...
function FilterBar({
  ...
  onFillDown, onBulkClose, onBulkReopen, onBulkDelete, onDeselect,
}: {
  search: string; onSearchChange: (v: string) => void;
  statusFilter: StatusFilter; onStatusChange: (v: StatusFilter) => void;
  budgetFilter: BudgetFilter; onBudgetChange: (v: BudgetFilter) => void;
  ...
  // When rows are selected, swap the filter controls for bulk-action controls
  // in the same bar — no layout shift.
  if (selectedCount > 0) {
    return (
      <div className="flex flex-wrap items-center gap-2 border-b border-border/40 bg-primary/5 px-2 py-1.5">
        ...
        <Button size="xs" variant="outline" onClick={onBulkClose}>Close All</Button>
        <Button size="xs" variant="outline" onClick={onBulkReopen}>Reopen All</Button>
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 border-b border-border/40 bg-muted/10 px-2 py-1.5">
      ...
      {hasFilters && (
        <button
          onClick={() => { onSearchChange(""); onStatusChange("all"); onBudgetChange("all"); }}
